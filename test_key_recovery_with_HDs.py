'''key recovery attack based on hierarchical distinguishers.
No GPUs are used.
'''
...
glut.generate_lookup_tables_for_key_recovery()
print('Done. Start test')

# the file for saving distinguishers
folder = './saved_model/new_inputs/'
# load 7-round distinguisher
net_paths = [
...
net7 = HD(nets=nets, bits=selected_bits, weights=weights)
...
net6 = HD(nets=nets, bits=selected_bits, weights=weights)


# m8 = np.load('data_wrong_key_8r_mean_1e6.npy')
# s8 = np.load('data_wrong_key_8r_std_1e6.npy'); s8 = 1.0/s8
m7 = np.load(folder + 'data_wrong_key_7r_mean.npy')
s7 = np.load(folder + 'data_wrong_key_7r_std.npy'); s7 = 1.0/s7
m6 = np.load(folder + 'data_wrong_key_6r_mean.npy')
s6 = np.load(folder + 'data_wrong_key_6r_std.npy'); s6 = 1.0/s6
...
def verifier_search(cts, best_guess, use_n=64, net=net6):
  ...
  # convert into new input
  t0, t1, t2 = pt0a ^ pt0b, pt1a ^ pt1b, pt0a ^ pt1a
...
def wrong_key_decryption(n, diff=(0x0040, 0x0), nr=7, net=net7):
  ...
  for i in range(2**16):
    ...
    # convert into new input
    t0, t1, t2 = c0a ^ c0b, c1a ^ c1b, c0a ^ c1a
    X = sp.convert_to_binary_for_new_input([t0, t1, t2])
...
def test(n, nr=11, num_structures=100, it=500, cutoff1=5.0, cutoff2=10.0, neutral_bits=[20,21,22,14,15,23], keyschedule='real',net=net7, net_help=net6, m_main=m7, s_main=s7,  m_help=m6, s_help=s6, verify_breadth=None):
  ...
  for i in range(n):
    ...
    print('the time consumption is {}'.format(t3 - t2))
  t1 = time();
  print("Done.");
...
arr1, arr2, good = test(100, cutoff1=5, cutoff2=10);
